...
PADDING_COLS_Y0 = {(z, 3) for z in range(28, N_Z)}  # y=0, x=3, z>=28
PADDING_COLS_Y1 = {(z, 3) for z in range(28, N_Z)}  # y=1, x=3, z>=28
...
PI0_FEAT_DIM  = N_Z * N_Y * N_X * 2
TOTAL_FEAT_DIM = INIT_FEAT_DIM + PI0_FEAT_DIM  # 3712
...
DEFAULT_MIN_BLUE = 20
DEFAULT_MAX_BLUE = 35
DEFAULT_BLUE_FILES = ["SHA3_512_all_blue.py"]
RED_BLUE_GAP = 5
...
def deterministic_full_forward(init_col_red, init_col_blue):
    ...
    feat = np.concatenate([
        init_col_red.astype(np.float32).flatten(),   # 256 (64x4)
        init_col_blue.astype(np.float32).flatten(),  # 256 (64x4)
        pi1_r,                                        # 1600
        pi1_b,                                        # 1600
    ])
...
def load_blue_solutions(blue_file=None, max_count=None):
    ...
    if blue_file is None:
        blue_file = os.path.join(os.path.dirname(os.path.dirname(_current_dir)),
                                 'data', 'sha3_512', 'SHA3_512_all_blue.py')
...
def worker_generate(params):
    ...
    while n_gen < n_target:
        try:
            model = gp.Model(f"Gen512_w{worker_id}")
            ...
            initial_state = [[[Bit(model, f"init_z{z}y{y}x{x}", "c")
            ...
            for z in range(N_Z):
                for x in range(4):
                    ...
                    if (z, x) in PADDING_COLS_Y0:
                        ...
                        initial_state[z][0][x] = Bit(model, f"init_z{z}_y0_x{x}", "c")
                    elif is_blue:
                    ...
                    if (z, x) in PADDING_COLS_Y1:
                        ...
                        initial_state[z][1][x] = Bit(model, f"init_z{z}_y1_x{x}", "c")
                    else:
            ...
            if model.SolCount > 0:
                ...
                init_blue = np.zeros((N_Z, 4), dtype=np.int8)
                red_positions = []
...
def main():
    parser = argparse.ArgumentParser(description='SHA3-512 Work 1 production training data generation')
    ...
    parser.add_argument('--min-red-gap', type=int, default=RED_BLUE_GAP,
                        help='minimum red excess over blue')
    ...
    if args.output is None:
        args.output = os.path.join(output_dir,
                                   f'sha3_512_train_{args.n_samples}.npz')

    print("=" * 60)
    print("SHA3-512 Work 1: Production Training Data Generation")
    print("=" * 60)
    print(f"Target samples: {args.n_samples:,}")
    print(f"Parallel processes: {args.n_procs}")
    print(f"Params: B[{args.min_blue},{args.max_blue}] "
          f"R>=B+{args.min_red_gap} <=B+R<={args.total_max}")
    print(f"MILP: gap={args.gap}, tlimit={args.time_limit}s")
    print(f"Feature dimension: {TOTAL_FEAT_DIM}")
    print(f"Samples per blue distribution: {args.n_per_blue}")
    print(f"Padding columns: {len(PADDING_COLS_Y0)} (y=0,y=1: x=3,z>=28)")
    print(f"Controllable columns: {N_CONTROLLABLE} (x=0..3, excluding padding)")
    ...
    if n_blue_unique < n_blue_needed:
        ...
        print(f"Generating {extra} additional random blue schemes")
