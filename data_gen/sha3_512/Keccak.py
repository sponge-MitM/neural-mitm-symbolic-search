...
def create_first_theta_operation(model, state, operation_name="theta"):
    ...
    C = [[Bit(model, f"{operation_name}_C_x{x}_z{z}", 'c') for x in range(5)] for z in range(64)]
    D = [[Bit(model, f"{operation_name}_D_x{x}_z{z}", 'c') for x in range(5)] for z in range(64)]
    for z in range(64):
        for x in range(5):
