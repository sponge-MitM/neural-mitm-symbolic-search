...
parser.add_argument("--least-number", type=int, default=22, help="minimum number of blue neutral bits")
...
print(f"=== [SHA3-512] Searching for blue bit count >= {least_number} ===")
print(f"  Seed: {seed}, time limit: {time_limit}s, target solutions: {max_solutions}")


PADDING_COLS = {(z, 3) for z in range(28, 64)}


model = gp.Model("Keccak512_MILP_BlueSearch")
...
for z in range(64):
    for x in range(4):
        ...
        blue_bits.append(initial_state[z][1][x].b)



total_positions = 64 * 4 - len(PADDING_COLS)
print(f"Blue bit variables: {len(blue_bits)} (y=0 + y=1, excluding padding)")
print(f"blue_vars dict size: {len(blue_vars)}")
print(f"Uncontrollable positions: all 64 x=4 columns + {len(PADDING_COLS)} padding columns")
...
ALL_POSITIONS = [(z, x) for z in range(64) for x in range(4)
                 if (z, x) not in PADDING_COLS]
...
print(f"Gurobi solve complete: {pool_count} solutions ({phase1_elapsed:.0f}s)")
...
if len(all_solutions) < max_solutions:
    print(f"\n=== Phase 2: GF(2) perturbation generation (top up to {max_solutions} solutions) ===")
    ...
    print(f"  GF(2) perturbation: {successes}/{attempts} succeeded ({phase2_elapsed:.0f}s)")
...
print(f"\n{'='*50}")
if len(all_solutions) >= max_solutions:
    print(f"OK Found {len(all_solutions)} solutions! (total {total_elapsed:.0f}s)")
elif len(all_solutions) > 0:
    print(f" Target {max_solutions} solutions, found {len(all_solutions)} (total {total_elapsed:.0f}s)")
else:
    print(f"x No solutions found (total {total_elapsed:.0f}s)")
...
if len(all_solutions) > 0:
    ...
    output_file = os.path.join(out_dir, "SHA3_512_all_blue.py")
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"all_solutions = {all_solutions}\n")
    print(f"OK Saved {len(all_solutions)} solutions to: {output_file}")
    print(f"  Each solution shape: 64x5 = {len(all_solutions[0])}x{len(all_solutions[0][0])} bits")
else:
    print("x No valid solutions found!")
    print("  Consider checking the constraints or trying different solver parameters")
