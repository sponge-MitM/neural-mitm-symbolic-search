...
PROCESSED_DIR = os.path.join(REPO_ROOT, 'data', 'sha3_384')
RESULT_DIR = PROCESSED_DIR
PREFIX = 'sha3_384_blue_labels_'
...
def main():
    ...
    ap.add_argument('--base-name', type=str, default=None,
                    help='feature file name prefix (default: latest sha3_384_blue_labels_*)')
    ...
    if args.base_name is None:
        ...
        for d in (PROCESSED_DIR, RESULT_DIR):
            ...
            for f in os.listdir(d):
                if f.startswith('sha3_384_blue_labels_') and f.endswith('_features.npy'):
                    candidates.append(os.path.join(d, f))
        if not candidates:
            raise FileNotFoundError("sha3_384_blue_labels_*_features.npy not found; "
