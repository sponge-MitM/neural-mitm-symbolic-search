...
def create_theta_operation(model, state, operation_name="theta"):
    ...
    for x in range(5):
        for z in range(64):
            ...
            xor_vars = xor_with_ul_input_no_delta_b(model, input_bits, C_bit, f"{operation_name}_C_x{x}_z{z}", need_has_ul=True)
    ...
    for x in range(5):
        for z in range(64):
            ...
            # Create XOR operation
            xor_vars = xor_with_ul_input_no_delta_b(model, [input_bit1, input_bit2], D_bit, f"{operation_name}_D_x{x}_z{z}", need_has_ul=True)
...
def create_chi_operation(model, state, operation_name="rho_east"):
    ...
    # Compute chi in one step (XAND unified encoding): new = x0  xor  (x1 and x2), eliminating the intermediate and_bit
    for z in range(64):
        for y in range(5):
            for x in range(5):
                x1 = (x + 1) % 5
                ...
                # XAND: y = x0  xor  (x1 and x2) (6+ fewer variables than the two-step AND+XOR combination)
                xand_vars = xand_operation(model, original_bit, bit1, bit2, new_bit, operation_name=new_bit_name)

                # Store new state and variables
                new_state[z][y][x] = new_bit
                chi_vars[f"new_z{z}_y{y}_x{x}"] = xand_vars
                # Backward-compatible interface: keep the and_ entry (the write script reads .get('quad', 0))
                chi_vars[f"and_z{z}_y{y}_x{x}"] = {'quad': 0}
